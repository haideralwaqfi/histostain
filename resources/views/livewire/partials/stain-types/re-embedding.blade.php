    ...
    @foreach($typeData['blocks'] ?? [] as $i => $block)
        <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 space-y-3">
            ...
            <div class="space-y-3">
                ...
                <x-form.textarea wire:model="typeData.blocks.{{ $i }}.reason" label="Reason for re-embedding" rows="2" required />
                @error("typeData.blocks.{$i}.reason") <x-form.error>{{ $message }}</x-form.error> @enderror
                <x-form.textarea wire:model="typeData.blocks.{{ $i }}.orientation_instructions" label="Orientation instructions" rows="2" />
            </div>
        </div>
    @endforeach
