    ...
    @foreach($typeData['blocks'] ?? [] as $i => $block)
        <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div><x-form.input wire:model="typeData.blocks.{{ $i }}.probe" label="Probe" required /></div>
                <div class="col-span-2"><x-form.textarea wire:model="typeData.blocks.{{ $i }}.indication" label="Indication" rows="2" required /></div>
                <div class="col-span-2"><x-form.input wire:model="typeData.blocks.{{ $i }}.send_out_lab" label="Send-out lab (optional)" /></div>
                <div class="col-span-2"><x-form.textarea wire:model="typeData.blocks.{{ $i }}.special_instructions" label="Special instructions" rows="2" /></div>
            </div>
        </div>
    @endforeach
